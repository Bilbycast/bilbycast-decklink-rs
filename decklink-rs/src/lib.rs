...
impl DecklinkPlayout {
    ...
    pub fn write_video(&mut self, data: &[u8], stream_time_90k: i64) -> Result<()> {
        ...
        self.map_video_write_result(rc, data.len(), stream_time_90k)
    }

    fn map_video_write_result(&self, rc: i32, data_len: usize, stream_time_90k: i64) -> Result<()> {
        match rc {
    ...
    ///
    /// `stream_time_90k` carries the same meaning and the same constraints as
    /// in [`write_video`](Self::write_video): the packets ride the frame, so
    /// they land wherever the frame's timestamp puts it.
